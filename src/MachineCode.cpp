...
NeonBinaryInstruction::NeonBinaryInstruction(MachineBlock* p, int op, MachineOperand* dst, MachineOperand* src1, MachineOperand* src2, int cond)
{
    this->parent = p;
    this->type = MachineInstruction::NEONBINARY;
    this->op = op;
    this->cond = cond;
    this->def_list.push_back(dst);
    this->use_list.push_back(src1);
...
void NeonBinaryInstruction::output()
{
    switch (this->op)
    {
    case NeonBinaryInstruction::ADD:
...
NeonldrInstrcution::NeonldrInstrcution(MachineBlock* p,MachineOperand* dst, MachineOperand* src1, MachineOperand* src2,int cond)
{
    this->parent = p;
    this->type = MachineInstruction::NEONLdr;
    this->op = -1;
    this->cond = cond;
    this->def_list.push_back(dst);
    this->use_list.push_back(src1);
...
void NeonldrInstrcution::output()
{
    ...
    // Load address,eg: vldr.32 r1, [src1 {,src2}]
    if(this->use_list[0]->isReg()||this->use_list[0]->isVReg())
        fprintf(yyout, "[");
...
NeonStrInstruction::NeonStrInstruction(MachineBlock* p,MachineOperand* src1, MachineOperand* src2, MachineOperand* src3, int cond)
{
    // TODO
    this->type = MachineInstruction::NEONStr;
    this->parent = p;
    this->cond = cond;
    this->use_list.push_back(src1);
    src1->setParent(this);
...
void NeonStrInstruction::output()
{
    // TODO
    fprintf(yyout, "\tstr ");
    //eg: vstr.32 src1, [src2 {,src3}]
    this->use_list[0]->output();
    fprintf(yyout, ", ");
    if(this->use_list[1]->isReg()||this->use_list[1]->isVReg())
    {
...
NeonMovInstruction::NeonMovInstruction(MachineBlock* p, int op,MachineOperand* dst, MachineOperand* src,int cond)
{
    this->type = MachineInstruction::MOV;
    this->parent = p;
    this->op = op;
...
void NeonMovInstruction::output()
{
    fprintf(yyout, "\tvmov");
    //PrintCond();
    fprintf(yyout, " ");
...
NeonCmpInstruction::NeonCmpInstruction(MachineBlock* p, MachineOperand* src1, MachineOperand* src2, int cond)
{
    // TODO
    this->type = MachineInstruction::NEONCMP;
    this->parent = p;
    this->cond = cond;
    p->setCmpCond(cond);
    this->use_list.push_back(src1);
...
void NeonCmpInstruction::output()
{
...
void MachineUnit::PrintGlobalDecl()
{
    ...
    for (long unsigned int i = 0; i < global_id_list.size(); i++) 
    {
        ...
        if(se->getType()->isInt()&&(se->getType()->whetherConst()))//浮点
        {
            //const常量
