...
public:
    ...
    */
    enum instType { BINARY, LOAD, STORE, MOV, BRANCH, CMP, STACK,NEONStr,NEONLdr,NEONBINARY,NEONCMP,NEONVCVT,NEONMOV};//挪这里
...
public:
    ...
    二元运算指令，加减乘除与或
    */
    enum opType { ADD, SUB, MUL, DIV, MOD,AND, OR };//arm貌似和取模？六种关系运算也不用？
...
class NeonStrInstruction: public MachineInstruction
{
    NeonStrInstruction(MachineBlock* p,MachineOperand* src1, MachineOperand* src2, MachineOperand* src3 = nullptr, int cond = MachineInstruction::NONE);
    void output();
};
class NeonldrInstrcution: public MachineInstruction
{
    NeonldrInstrcution(MachineBlock* p,MachineOperand* dst, MachineOperand* src1, MachineOperand* src2 = nullptr, int cond = MachineInstruction::NONE);
    void output();
};

class NeonBinaryInstruction:public MachineInstruction
{
public:
    enum opType { ADD, SUB, MUL, DIV, MOD,AND, OR };
    NeonBinaryInstruction(MachineBlock* p, int op, MachineOperand* dst, MachineOperand* src1, MachineOperand* src2, int cond = MachineInstruction::NONE);
    void output();
};

class NeonCmpInstruction:public MachineInstruction
{
    //Neon指令中比较指令有两种vcmp和vcmpe，暂时不确定有什么具体区别，先只考虑一种
    enum opType { CMP };
    NeonCmpInstruction(MachineBlock* p, MachineOperand* src1, MachineOperand* src2, int cond = MachineInstruction::NONE);
    void output();
};
...
class NeonMovInstruction:public MachineInstruction
{
    NeonMovInstruction(MachineBlock* p, int op, MachineOperand* dst, MachineOperand* src,int cond = MachineInstruction::NONE);
    void output();
};
